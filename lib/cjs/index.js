...
class ColourDecoder {
  ...
  static handleLine(line, beatmap) {
    ...
    const colour = new osuClasses.Colour(value[0], value[1], value[2]);
...
class HitObjectDecoder {
  ...
  static *convertPoints(points, endPoint, isFirst, offset) {
    ...
    function readPoint(point, offset) {
      ...
      const pos = new osuClasses.Vector2(coords[0], coords[1]).subtract(offset);
...
class EventDecoder {
  ...
  static _getEventType(input) {
    ...
    eventType = isFinite(eventType) ? eventType : osuClasses.EventType[input];

    if (osuClasses.EventType[eventType]) {
      return eventType;
    }
...
class StoryboardDataDecoder {
  ...
  static handleElement(line) {
    ...
    eventType = isFinite(eventType)
      ? eventType
      : osuClasses.EventType[data[0]];

    const index = eventType === osuClasses.EventType.Sample ? 2 : 1;
    let layerType = parseInt(data[index]);

    layerType = isFinite(layerType)
      ? layerType
      : osuClasses.LayerType[data[index]];

    switch (eventType) {
      case osuClasses.EventType.Sprite: {
        const element = new osuClasses.StoryboardSprite();
        ...
        element.origin = isFinite(element.origin)
          ? element.origin
          : osuClasses.Origins[data[2]];

        return element;
      }
      case osuClasses.EventType.Animation: {
        const element = new osuClasses.StoryboardAnimation();
        ...
        element.loop = isFinite(element.loop)
          ? element.loop
          : osuClasses.LoopType[data[8]];

        return element;
      }
      case osuClasses.EventType.Sample: {
        const element = new osuClasses.StoryboardSample();
    ...
    return new osuClasses.StoryboardSprite();
  }
  /**
  ...
  static handleLoop(line) {
    const data = line.split(',');
    const loop = new osuClasses.CommandLoop();
  ...
  static handleTrigger(line) {
    const data = line.split(',');
    const trigger = new osuClasses.CommandTrigger();
  ...
  static handleCommand(line) {
    ...
    switch (commandType) {
      case osuClasses.CommandType.Fade:
        command = new osuClasses.FadeCommand();
      ...
      case osuClasses.CommandType.Movement:
        command = new osuClasses.MoveCommand();
      ...
      case osuClasses.CommandType.MovementX:
        command = new osuClasses.MoveXCommand();
        command.startX = Parsing.parseFloat(data[4]);
      ...
      case osuClasses.CommandType.MovementY:
        command = new osuClasses.MoveYCommand();
      ...
      case osuClasses.CommandType.Scale:
        command = new osuClasses.ScaleCommand();
      ...
      case osuClasses.CommandType.VectorScale:
        command = new osuClasses.VectorScaleCommand();
      ...
      case osuClasses.CommandType.Rotation:
        command = new osuClasses.RotateCommand();
        ...
        break;
      case osuClasses.CommandType.Colour:
        command = new osuClasses.ColourCommand();
  ...
  static handleParameterCommand(data) {
    ...
    switch (parameterType) {
      case osuClasses.ParameterType.HorizontalFlip:
        command = new osuClasses.HorizontalFlipCommand();
        break;
      case osuClasses.ParameterType.VerticalFlip:
        command = new osuClasses.VerticalFlipCommand();
        break;
      default:
        command = new osuClasses.BlendingCommand();
    }
...
class StoryboardDecoder {
  ...
  _parseDepth0(line, storyboard) {
    ...
    if (this._element instanceof osuClasses.StoryboardSample) {
      storyboard.getLayer(osuClasses.LayerType.Samples).push(this._element);
...
class HitObjectEncoder {
  ...
  static encodeHitObjects(beatmap) {
    ...
    hitObjects.forEach((hitObject) => {
      ...
      const set = [];
      const normal = hitObject.samples.find((s) => s.hitSound === osuClasses.HitSound[osuClasses.HitSound.Normal]);
      const addition = hitObject.samples.find((s) => s.hitSound !== osuClasses.HitSound[osuClasses.HitSound.Normal]);
      ...
      const extrasLine = hitObject.hitType & osuClasses.HitType.Hold ? extras.join(':') : extras.join(',');
...
class EventsEncoder {
  ...
  static encodeStoryboardElement(element) {
    ...
    if (element instanceof osuClasses.StoryboardSprite) {
      ...
      encoded.push('Sprite');
      encoded.push(osuClasses.LayerType[sprite.layer]);
      encoded.push(osuClasses.Origins[sprite.origin]);
      encoded.push(`"${sprite.filePath}"`);
      encoded.push(`${sprite.startPosition}`);
    }
    else if (element instanceof osuClasses.StoryboardAnimation) {
      const animation = element;

      encoded.push('Animation');
      encoded.push(osuClasses.LayerType[animation.layer]);
      encoded.push(osuClasses.Origins[animation.origin]);
    ...
    else if (element instanceof osuClasses.StoryboardSample) {
  ...
  static encodeCompound(compound) {
    ...
    else if (compound instanceof osuClasses.CommandTrigger) {
    ...
    switch (command.type) {
      ...
      case osuClasses.CommandType.VectorScale: {
        ...
        if (scale.startScale.equals(scale.endScale)) {
          return scale.type === osuClasses.CommandType.Scale
            ? `${scale.startScale.x}`
            : `${scale.startScale}`;
        }
      ...
      case osuClasses.CommandType.MovementY: {
        ...
        if (move.type === osuClasses.CommandType.Movement) {
